...
bfile_list = ["gs://control_repo_by_platform/Axiom_KP_UCSF_EUR/gera/gera_ready4QC_AF_HRC_sa/gera.all.final"]
output_root_directories = ["gs://unicorn-qc/pre-imputation-qc/cohort-qc/gera_eur"]
...
def __main__(bfile:str, mt_1kg_path: str, mt_1kg_eur_path: str, output_root_directory: str,
    ...
    mt = hl.read_matrix_table(directory_structure["share"]+"/all.mt")
    pops = mt.aggregate_cols(hl.agg.counter(mt.pop))
...
######################################################################################################
input_pandas = pd.DataFrame({'bfile': bfile_list, 'out_directory': output_root_directories})
